...
export function PerformanceRecordForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 筋力測定の場合、相対1RMと測定時体重を追加
    const isStrengthTest = selectedTestType && ['bench_press', 'back_squat', 'deadlift'].includes(selectedTestType.name);
    ...
    <>
      ...
      {selectedTestType && Array.isArray(selectedTestType.fields) && (
        <>
          ...
          {/* 筋力測定の場合、相対1RMと測定時体重を表示 */}
          {selectedTestType && ['bench_press', 'back_squat', 'deadlift'].includes(selectedTestType.name) && (
